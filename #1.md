...
- Application's memory can be divided into 4 segmets
    - Code (Text)
...
- Allocate Block of memory
    ...
    - `realloc` - signature/definition of it is like : `void* realloc(void *ptr,size_t)`
...
> In `realloc` function, if the new size is greater than the old one, here the compiler see if it can extend the old block it will do that
> otherwise it will create another block of memory with new size and then copy all the bytes of the old block to it & then free the old block
 ...
 */
````
> As you can see in this case, the old block of code is extended to store more elements, that's because the address of `arr2` is like address `arr` <br>
